...
with open('goats.txt') as ifile, open('answer.txt', 'w') as ofile:
    txt = [*map(str.strip, ifile.readlines())]
    keys = txt[txt.index('COLOURS') + 1: txt.index('GOATS')]
    d = dict.fromkeys(keys, 0)
    for goat in txt[txt.index('GOATS') + 1:]:
        d[goat] += 1
    total = sum(d.values())
    print(*sorted([key for key, value in d.items() if value * 100 > 7 * total]), sep='\n', file=ofile)
